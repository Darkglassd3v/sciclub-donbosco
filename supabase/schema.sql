...
-- ---------------------------------------------------------------------------
-- Creare un account dal pannello Utenti
--
-- Scrive l'account direttamente in auth.users, con la password già cifrata e
-- la mail già segnata come confermata: la persona entra subito con email e
-- password, e non parte nessun messaggio.
--
-- Le due alternative non andavano bene. L'Admin API vuole la service_role,
-- che scavalca la RLS e in un sito statico sarebbe pubblica. signUp() con la
-- chiave anon obbliga ad aprire le registrazioni a chiunque abbia quella
-- chiave — che sta in chiaro in config.js, in un repository pubblico — e per
-- giunta manda una mail di conferma che il piano gratuito esaurisce dopo
-- pochi account ("email rate limit exceeded"). Così invece le registrazioni
-- restano chiuse: gli account nascono solo da qui, e solo per mano di un
-- superadmin.
--
-- security definer perché auth.users non è scrivibile da un utente normale;
-- il controllo su chi chiama è la prima cosa che fa il corpo della funzione.
-- ---------------------------------------------------------------------------
...
as $$
declare
  nuovo_id uuid := gen_random_uuid();
begin
  if not public.has_role('superadmin') then
    raise exception 'Solo un superadmin può creare account.';
  end if;

  if ruolo not in ('ospite', 'kiosk', 'utente', 'admin', 'superadmin') then
    raise exception 'Ruolo non valido: %.', ruolo;
  end if;

  email_nuovo := lower(trim(coalesce(email_nuovo, '')));
  if email_nuovo !~ '^[^@[:space:]]+@[^@[:space:]]+\.[^@[:space:]]+$' then
    raise exception 'Email non valida: %.', email_nuovo;
  end if;

  if length(coalesce(password_nuova, '')) < 8 then
    raise exception 'La password deve avere almeno 8 caratteri.';
  end if;

  if exists (select 1 from auth.users where email = email_nuovo) then
    raise exception 'Esiste già un account con questa email.';
  end if;

  -- I campi token vanno a stringa vuota e non a null: certe versioni di
  -- GoTrue li leggono senza aspettarsi un null e sbagliano il login.
  insert into auth.users (
    ...
    '00000000-0000-0000-0000-000000000000', nuovo_id, 'authenticated', 'authenticated',
    email_nuovo, crypt(password_nuova, gen_salt('bf')),
    now(), now(), now(),
    '{"provider":"email","providers":["email"]}'::jsonb, '{}'::jsonb,
    '', '', '', ''
  );

  -- Senza la riga in auth.identities l'account esiste ma il login con
  -- password viene rifiutato: è lì che GoTrue cerca il collegamento fra
  -- l'utente e il modo in cui accede.
  insert into auth.identities (
    id, user_id, provider_id, identity_data, provider,
    last_sign_in_at, created_at, updated_at
  ) values (
    gen_random_uuid(), nuovo_id, nuovo_id::text,
    jsonb_build_object('sub', nuovo_id::text, 'email', email_nuovo),
    'email', now(), now(), now()
  );

  -- Il trigger on_auth_user_created ha appena messo la riga profiles a
  -- 'ospite': qui prende il ruolo scelto da chi sta creando l'account.
  update public.profiles set role = crea_utente.ruolo where user_id = nuovo_id;

  return nuovo_id;
end;
$$;

comment on function public.crea_utente is 'Crea un account Supabase Auth già confermato. Solo per superadmin.';

-- La funzione scavalca la RLS per definizione: non deve poterla chiamare chi
-- non ha fatto login. Il controllo sul ruolo sta comunque dentro al corpo.
revoke all on function public.crea_utente(text, text, text) from public;
grant execute on function public.crea_utente(text, text, text) to authenticated;
