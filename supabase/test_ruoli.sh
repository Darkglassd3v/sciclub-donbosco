...
do $$ begin
  assert (select count(*) from public.profiles) = 4, 'il trigger non ha creato i profiles';
  assert (select role from public.profiles where email = 'utente@test') = 'utente',
         'il ruolo di partenza non è utente';
end $$;
...
-- Gerarchia kiosk < utente < admin < superadmin, letta sotto RLS: se
...
do $$ begin
  ...
  perform set_config('test.uid', '22222222-2222-2222-2222-222222222222', false);
  assert (select count(*) from public.profiles) = 4, 'il superadmin non vede tutti i profiles';
end $$;
